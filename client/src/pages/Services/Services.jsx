...
const Services = () => {
  ...
  const present = students.filter((student) => {
    return student.attendance && student.attendance.some(a => 
      new Date(a.date).toDateString() === new Date().toDateString() && a.present
    );
  })
  ...
  const service = [
    ...
    {
      title: `Total Student present in Class Today : ${present.length}`,
      link: "/add-student",
      button: ""
    }]
